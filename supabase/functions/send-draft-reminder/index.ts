...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Find professionals with status "rascunho" created more than 3 days ago
    ...
    if (!professionals || professionals.length === 0) {
      return new Response(JSON.stringify({ message: "No draft reminders to send", count: 0 }), {
        status: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    for (const prof of professionals) {
      ...
      const html = emailWrapper(`
        ...
        <p style="color:#374151;font-size:16px;line-height:1.7;margin:0 0 12px;">
          Seu perfil na <strong style="color:#b8396b;">Vitrine dos Especialistas da Beleza</strong> ainda não foi publicado.
        </p>
        <p style="color:#374151;font-size:16px;line-height:1.7;margin:0 0 12px;">
          Seu perfil está em <strong>rascunho</strong> e precisa ser enviado para aprovação. Complete as informações do seu perfil — foto, serviços e portfólio — e envie para que possamos publicá-lo!
        </p>
        <p style="color:#374151;font-size:16px;line-height:1.7;margin:0 0 12px;">
          Quanto antes você enviar, mais rápido seus clientes poderão encontrá-lo(a). 🚀
        </p>
        ${brandButton("Completar meu perfil", "https://vitrine.escola.ro/dashboard")}
      ...
      const res = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          ...
          to: [profile.email],
          subject: "📝 Seu perfil ainda não foi publicado — Complete e envie para aprovação!",
          html,
        }),
      });
